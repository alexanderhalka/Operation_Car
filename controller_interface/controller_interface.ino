...
const int MOTOR_RIGHT_PIN = 5;    // PWM pin for right turn
...
void loop() {
  if (Serial.available() > 0) {
    ...
    //forward 100% makes it go backwards
  // forward 0% makes it forwards 
    ...
    if (input.startsWith("F ")) {  // Forward command
      ...
      int speed = input.substring(2).toInt();
      speed = constrain(speed, 0, 255);
      analogWrite(MOTOR_LEFT_PIN, 255-speed);
      analogWrite(MOTOR_RIGHT_PIN, 0);
    }
    else if (input.startsWith("R ")) {  // Right command
      int speed = input.substring(2).toInt();
      speed = constrain(speed, 0, 255);
      analogWrite(MOTOR_RIGHT_PIN, 255-speed);
